...
filepath <- "/Users/shuowenchen/desktop/Research/Panel-Cross-over-jackknife/Simulation/R"
setwd(filepath)
...
# specification for simulated data
spec_fe.s <- lfp.s ~ laglfp.s + kids0_2 + kids3_5 + kids6_17 + loghusbandincome + age + age2 + factor(id) + factor(year) - 1

# Check number of missing LFP values
nomissing <- !is.na(lfp)
pop <- length(lfp[nomissing])
...
index <- deltahatfete$linear.predictors # predicted values within the link function
index0 <- index - betahatfete[1] * laglfp[nomissing] # predicted values WITHOUT lagged dependent variable (STATIC)
index1 <- index + betahatfete[1] * (1 - laglfp[nomissing]) # index0 + coef of laglfp

# Estimated beta coefficient from the data
beta0 <- betahatfete[c(1:P)]

true.index0 <- matrix(index0, nrow = T, byrow = FALSE)
true.index1 <- matrix(index1, nrow = T, byrow = FALSE)
...
se.betasfete <- matrix(0, nrow = B, ncol = P)
...
for (i in 1:B) {
  ...
  lfp.s <- matrix(0, nrow = T0, ncol = N0)
  lfp0 <- laglfp[year == 1] #initial value
  for (t in 1:T0) {
    # Interpretation: lfp0*betahatfete[1] + X'*betahatfete[2:end] > rnorm(N0),
    # Determines the lfp in the next period
    lfp.s[t, ] <- (lfp0 * true.index1[t, ] + (1 - lfp0) * true.index0[t, ] > rnorm(N0))
    lfp0 <- lfp.s[t, ] # update so as to simulate the next period lfp
  }
  # generate simulated lagged dependent variable
  laglfp.s <- matrix(rbind(laglfp[year == 1], lfp.s[-T0, ]), ncol = 1, byrow = FALSE)
  lfp.s <- matrix(lfp.s, ncol = 1, byrow = FALSE)
  # All other covariates are taken from the original data
  data.s <- data.frame(lfp.s = lfp.s, laglfp.s = laglfp.s, kids0_2 = kids0_2, kids3_5 = kids3_5, 
                       kids6_17 = kids6_17, loghusbandincome = loghusbandincome, age = age, 
                       age2 = age2, id = id, year = year)
  # ELIMINATING INDIVIDUALS WITH ALL 1's OR 0's
  # tapply(lfp.s, D, sum): counts LFP for each individual over the spell of the years (0-9)
  td <- kronecker(tapply(lfp.s, D, sum),  matrix(1, T0, 1)) 
  insample <- ((td > 0) & (td < T0))
  ...
  deltahatfete <- spglm(spec_fe.s, data = data.s[insample, ], family = binomial(link = "probit"), x = TRUE)
  ...
  betahatabc <- t(betahatfete - bias_dynamic_parameter(N, T0, deltahatfete$linear.predictors, 
                                                        deltahatfete$x[, 1:P], lfp.s[insample], 
                                                        L2 = L2, L3 = L3, L4 = L4))               
  ...
  # i = 1, ..., N/2, j = 1, ..., T
  index <- (id <= median(id[nomissing & insample]))
  deltahat11 <- spglm(spec_fe.s, data = data.s[index, ], family = binomial(link = "probit"))
  betahat11 <- deltahat11$coef[1:P]
  # i = N/2 + 1, ..., N, t = 1, ..., T
  index <- (id >= median(id[nomissing & insample]))  
  deltahat12 <- spglm(spec_fe.s, data = data.s[index, ], family = binomial(link = "probit"))
  betahat12 <- deltahat12$coef[1:P]
  
  # LEAVING HALF PANEL OUT, ALL INDIVIDUALS
  # i = 1, ..., N, j = 1, ..., T/2
  index <- (year <= median(year[nomissing & insample]))  
  deltahat21 <- spglm(spec_fe.s, data = data.s[index, ], family = binomial(link = "probit"))
  betahat21 <- deltahat21$coef[1:P]
  # i = 1, ..., N, j = T/2 + 1, ..., T
  index <- (year >= median(year[nomissing & insample]))  
  deltahat22 <- spglm(spec_fe.s, data = data.s[index, ], family = binomial(link = "probit"))
  betahat22 <- deltahat22$coef[1:P]
  # JBC (Fernandez-Val and Weidner, 2016)
...
# CROSS-OVER JACKKNIFE;
  ...
  index <- (id <= median(id[nomissing & insample]) & year <= median(year[nomissing & insample])) | 
    (id >= median(id[nomissing & insample]) & year >= median(year[nomissing & insample]))
  deltahat1 <- spglm(spec_fe.s, data = data.s[index, ], family = binomial(link = "probit"))
  betahat1 <- deltahat1$coef[1:P]
  # SECOND CROSS: i = 1, ..., N/2,  j = T/2 + 1, ..., T; i = N/2 + 1, ..., N, j = 1, ..., T/2
  index <- (id <= median(id[nomissing & insample]) & year >= median(year[nomissing & insample])) | 
    (id >= median(id[nomissing & insample]) & year <= median(year[nomissing & insample]))
  deltahat2 <- spglm(spec_fe.s,  data = data.s[index, ], family = binomial(link = "probit"))
  betahat2 <- deltahat2$coef[1:P]
  # CBC
  betahatcbc <- 2*betahatfete - (betahat1 + betahat2)/2    
  # STANDARD ERRORS;
  se.betahatfete <- summary(deltahatfete)$coeff[1:P, 2]
...
for (ind in 1:P) {
  ...
  table.beta[1,4] <- (mean((se.betasfete[conver, ind]/sd(betasfete[conver, ind]))))
  table.beta[1,5] <- (mean((betasfete[conver, ind] + qnorm(.05/2) * se.betasfete[conver, ind] <= beta0[ind]) &
                             (betasfete[conver, ind] + qnorm(1 - .05/2) * se.betasfete[conver, ind] >= beta0[ind])))
  ...
  table.beta[2,4] <- (mean((se.betasfete[conver, ind]/sd(betasabc[conver, ind]))))
  table.beta[2,5] <- (mean((betasabc[conver, ind] + qnorm(.05/2) * se.betasfete[conver, ind] <= beta0[ind]) & 
                             (betasabc[conver, ind] + qnorm(1 - .05/2) * se.betasfete[conver, ind] >= beta0[ind])))
  ...
  table.beta[3,4] <- (mean((se.betasfete[conver, ind]/sd(betasjbc[conver, ind]))))
  table.beta[3,5] <- (mean((betasjbc[conver, ind] + qnorm(.05/2) * se.betasfete[conver, ind] <= beta0[ind]) & 
                             (betasjbc[conver, ind] + qnorm(1 - .05/2) * se.betasfete[conver, ind] >= beta0[ind])))
  ...
  table.beta[4,4] <- (mean((se.betasfete[conver, ind]/sd(betascbc[conver, ind]))))
  table.beta[4,5] <- (mean((betascbc[conver, ind] + qnorm(.05/2) * se.betasfete[conver, ind] <= beta0[ind]) & 
                             (betascbc[conver, ind] + qnorm(1 - .05/2) * se.betasfete[conver, ind] >= beta0[ind])))
