# probit-LFP-MC file using feglm
...
spec_fe_s <- lfp_s ~ laglfp_s + kids0_2 + kids3_5 + kids6_17 + loghusbandincome + age + age2 | id + year
...
for (i in 1:B) {
  ...
  deltahatfete <- feglm(spec_fe_s, data = data_s[insample, ], family = binomial(link = "probit"))
  betahatfete <- deltahatfete$coefficients
  
  # ANALYTICAL BIAS CORRECTION;
  betahatabc <- t(betahatfete - bias_dynamic_parameter(N, T0, predict(deltahatfete), 
                                                       as.matrix(deltahatfete$data[, 2:(P+1)]), lfp_s[insample], 
                                                       L2 = L2, L3 = L3, L4 = L4))               
  ...
  deltahat11 <- feglm(spec_fe_s, data = data_s[index, ], family = binomial(link = "probit"))
  betahat11 <- deltahat11$coefficients
  # i = N/2 + 1, ..., N, t = 1, ..., T
  index <- (data_s$id >= median(data_s$id[nomissing & insample]))  
  deltahat12 <- feglm(spec_fe_s, data = data_s[index, ], family = binomial(link = "probit"))
  ...
  deltahat21 <- feglm(spec_fe_s, data = data_s[index, ], family = binomial(link = "probit"))
  betahat21 <- deltahat21$coefficients
  # i = 1, ..., N, j = T/2 + 1, ..., T
  index <- (data_s$year >= median(data_s$year[nomissing & insample]))  
  deltahat22 <- feglm(spec_fe_s, data = data_s[index, ], family = binomial(link = "probit"))
  ...
  deltahat1 <- feglm(spec_fe_s, data = data_s[index, ], family = binomial(link = "probit"))
  betahat1 <- deltahat1$coefficients
  # SECOND CROSS: i = 1, ..., N/2,  j = T/2 + 1, ..., T; i = N/2 + 1, ..., N, j = 1, ..., T/2
  index <- (data_s$id <= median(data_s$id[nomissing & insample]) & data_s$year >= median(data_s$year[nomissing & insample])) | 
    (data_s$id >= median(data_s$id[nomissing & insample]) & data_s$year <= median(data_s$year[nomissing & insample]))
  deltahat2 <- feglm(spec_fe_s,  data = data_s[index, ], family = binomial(link = "probit"))
