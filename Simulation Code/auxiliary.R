...
spec_fe_s <- lfp ~ laglfp + kids0_2 + kids3_5 + kids6_17 + loghusbandincome + age + age2 + factor(id) + factor(year) - 1
...
boot_test <- function(data, spec_fe_s) {
  return(c(data[,2]))
}
boot_stat <- function(data, spec_fe_s) {
  ...
  deltahatfete <- spglm(spec_fe_s, data = data[insample, ], family = binomial(link = "probit"), x = TRUE)
  ...
  betahatabc <- t(betahatfete - bias_dynamic_parameter(N, T0, deltahatfete$linear.predictors, 
                                                       deltahatfete$x[, 1:P], data$lfp[insample], 
                                                       L2 = L2, L3 = L3, L4 = L4))               
  ...
  deltahat11 <- spglm(spec_fe_s, data = data[index, ], family = binomial(link = "probit"))
  betahat11 <- deltahat11$coef[1:P]
  # i = N/2 + 1, ..., N, t = 1, ..., T
  index <- (data$id >= median(data$id[nomissing & insample]))  
  deltahat12 <- spglm(spec_fe_s, data = data[index, ], family = binomial(link = "probit"))
  betahat12 <- deltahat12$coef[1:P]
  
  # LEAVING HALF PANEL OUT, ALL INDIVIDUALS
  # i = 1, ..., N, j = 1, ..., T/2
  index <- (data$year <= median(data$year[nomissing & insample]))  
  deltahat21 <- spglm(spec_fe_s, data = data[index, ], family = binomial(link = "probit"))
  betahat21 <- deltahat21$coef[1:P]
  # i = 1, ..., N, j = T/2 + 1, ..., T
  index <- (data$year >= median(data$year[nomissing & insample]))  
  deltahat22 <- spglm(spec_fe_s, data = data[index, ], family = binomial(link = "probit"))
  ...
  deltahat1 <- spglm(spec_fe_s, data = data[index, ], family = binomial(link = "probit"))
  betahat1 <- deltahat1$coef[1:P]
  # SECOND CROSS: i = 1, ..., N/2,  j = T/2 + 1, ..., T; i = N/2 + 1, ..., N, j = 1, ..., T/2
  index <- (data$id <= median(data$id[nomissing & insample]) & data$year >= median(data$year[nomissing & insample])) | 
    (data$id >= median(data$id[nomissing & insample]) & data$year <= median(data$year[nomissing & insample]))
  deltahat2 <- spglm(spec_fe_s,  data = data[index, ], family = binomial(link = "probit"))
